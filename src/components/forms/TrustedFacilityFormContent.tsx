...
export type TrustedFacilityFormContentProps = React.HTMLProps<HTMLDivElement> & {
    submissionStatus: number | undefined
    setSubmissionStatus: React.Dispatch<React.SetStateAction<number | undefined>>
}

export const TrustedFacilityFormContent: React.FC<TrustedFacilityFormContentProps> = ({
    submissionStatus,
    setSubmissionStatus
}) => {
    const {
    ...
    } = useForm<TrustedFacilityFormData>({
        mode: 'onChange'
    });
    ...
    return (
        ...
        >
            ...
            <div className={clsx(
                ...
                <TextField className="flex flex-col gap-1" isInvalid={errors.phone != undefined}>
                    <Label 
                        className="font-medium text-sm text-light" 
                        htmlFor="input-phone"
                    >
